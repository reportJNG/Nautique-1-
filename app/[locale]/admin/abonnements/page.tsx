...
export default async function AdminAbonnementsPage({
  ...
  const stats = {
    ...
    actif: abonnements.filter((abonnement) => abonnement.statut === "ACT").length,
    attente: abonnements.filter((abonnement) => abonnement.statut === "ATT").length,
    expire: abonnements.filter((abonnement) => abonnement.statut === "EXP").length,
  };
  ...
  const getStatusConfig = (statut: string) => {
    ...
    > = {
      ACT: {
        icon: CheckCircle,
        badgeClass: "text-cyan-400 ring-cyan-400/30",
        label: t("abonnementStatus.ACT"),
      },
      ATT: {
        icon: Clock,
        badgeClass: "text-accent ring-accent/30",
      ...
      ANN: {
        icon: XCircle,
        badgeClass: "text-muted-foreground ring-border/30",
  ...
  const statCards = [
    {
      label: t("abonnementsUi.stats.total"),
      value: stats.total,
    ...
    {
      label: t("abonnementsUi.stats.active"),
      value: stats.actif,
    ...
    {
      label: t("abonnementsUi.stats.pending"),
      value: stats.attente,
    ...
    {
      label: t("abonnementsUi.stats.expired"),
      value: stats.expire,
  ...
  return (
    <AdminPageShell locale={locale}>
      <div className="space-y-0">
        <div className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-[#042C53] via-[#0C447C] to-[#185FA5] px-8 pb-16 pt-8">
          <svg
          ...
          <div className="overflow-hidden rounded-2xl border border-border/50 bg-transparent shadow-sm">
            <div className="flex flex-col gap-3 border-b border-border/50 px-6 py-4 sm:flex-row sm:items-center sm:justify-between">
              <div>
                <h2 className="text-base font-semibold text-foreground">
                  {t("abonnementsUi.listTitle")}
                </h2>
                <p className="text-xs text-foreground0">
                  {t("abonnementsUi.resultsCount", { count: stats.total })}
                </p>
              </div>
              <div className="flex w-full items-center gap-2 rounded-xl border border-border/50 bg-card/30 px-3 py-2 text-sm text-muted-foreground sm:w-56">
                <Search className="h-3.5 w-3.5 flex-shrink-0" />
                <span className="text-xs">{t("abonnementsUi.searchPlaceholder")}</span>
              </div>
            </div>
              ...
              <table className="w-full text-sm">
                ...
                <tbody className="divide-y divide-slate-700/50">
                  {abonnements.map((abonnement) => {
                    ...
                    const { icon: StatusIcon, badgeClass, label } = getStatusConfig(
                      abonnement.statut,
                    );
                    ...
                    return (
                      ...
                      >
                        ...
                        <td className="px-5 py-3.5">
                          <span
                            className={`inline-flex items-center gap-1.5 rounded-full bg-transparent px-2.5 py-1 text-[11px] font-semibold ring-1 ${badgeClass}`}
                          >
                        ...
                        <td className="px-5 py-3.5 text-right">
                          <span className="font-bold tabular-nums text-foreground">
                            ...
                            <span className="text-xs font-medium text-foreground0">DA</span>
                          </span>
                        </td>
            ...
            {abonnements.length === 0 && (
              <div className="flex flex-col items-center justify-center gap-3 py-16">
                ...
                <p className="text-sm text-foreground0">{t("abonnementsUi.empty")}</p>
              </div>
            )}
          </div>
        </div>
