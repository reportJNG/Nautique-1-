...
export function AccesClient() {
  ...
  return (
    <>
      <style>{`
        .acc-scanner-card {
          border-radius: 14px; border: 1px solid rgba(6,182,212,0.15);
        ...
        .acc-scanner-title { font-size: 14px; font-weight: 600; color: #e2f0ff; }
        .acc-scanner-desc  { font-size: 12px; color: #7a93b4; margin-top: 1px; }
        ...
        .acc-search-icon { position: absolute; left: 12px; top: 50%; transform: translateY(-50%); color: #4a6280; pointer-events: none; }
        .acc-search-input {
          width: 100%; padding: 10px 14px 10px 38px;
          background: rgba(255,255,255,0.04); border: 1px solid rgba(255,255,255,0.08);
          border-radius: 9px; color: #e2f0ff; font-size: 14px; outline: none; box-sizing: border-box;
          transition: border-color 180ms ease, box-shadow 180ms ease;
        }
        .acc-search-input::placeholder { color: #4a6280; }
        .acc-search-input:focus { border-color: rgba(6,182,212,0.4); box-shadow: 0 0 0 3px rgba(6,182,212,0.08); }
        .acc-validate-btn {
        ...
        .acc-status-dot {
          ...
          box-shadow: 0 0 8px rgba(16,185,129,0.5); animation: acc-pulse 2s infinite;
        }
        @keyframes acc-pulse { 0%, 100% { opacity: 1; } 50% { opacity: 0.5; } }
        .acc-status-text { font-size: 12.5px; color: #7a93b4; }
        .acc-status-text strong { color: #10b981; }

        .acc-stats-grid { display: grid; gap: 12px; grid-template-columns: repeat(3, 1fr); margin-bottom: 0; }
        .acc-stat {
          border-radius: 10px; padding: 14px 16px;
          border: 1px solid rgba(255,255,255,0.05); background: rgba(13,21,38,0.5); text-align: center;
        }
        .acc-stat-val { font-size: 22px; font-weight: 800; color: #f0f9ff; letter-spacing: -0.02em; }
        .acc-stat-lbl { font-size: 10.5px; font-weight: 700; text-transform: uppercase; letter-spacing: 0.08em; color: #4a6280; margin-top: 3px; }
        ...
        .acc-jname { font-size: 13px; font-weight: 600; color: #e2f0ff; }
        .acc-jdossier { font-size: 11.5px; color: #4a6280; font-family: monospace; }
        .acc-jtime { font-size: 12px; color: #4a6280; font-variant-numeric: tabular-nums; }
        .acc-granted { color: #10b981; }
        .acc-denied  { color: #f87171; }
        ...
        .acc-empty-icon {
          ...
          display: flex; align-items: center; justify-content: center; color: #3a546e;
        }
        .acc-empty-text { font-size: 13.5px; color: #4a6280; }
      `}</style>

      {/* Stats row */}
      <div className="acc-stats-grid">
        <div className="acc-stat">
          <div className="acc-stat-val" style={{ color: "#10b981" }}>{stats.granted}</div>
          <div className="acc-stat-lbl">Entrées aujourd&apos;hui</div>
        </div>
        <div className="acc-stat">
          <div className="acc-stat-val" style={{ color: "#f87171" }}>{stats.denied}</div>
          <div className="acc-stat-lbl">Refusés</div>
        </div>
        <div className="acc-stat">
          <div className="acc-stat-val" style={{ color: "#06b6d4" }}>{journal.length}</div>
      ...
      <AdminSection
        ...
        headerRight={
          <span style={{ fontSize: 11, color: "#4a6280", display: "flex", alignItems: "center", gap: 4 }}>
            <Activity size={12} />
            Temps réel
          </span>
        }
