...
import { LayoutDashboard, Users, Calendar, ClipboardList, Dumbbell, UserCheck, UserCog, CreditCard, DoorOpen, Settings, Menu, LogOut, Waves, ChevronRight, } from "lucide-react";
...
export function AdminSidebar({ agent, locale }: AdminSidebarProps) {
    ...
    const filteredNavItems = navItems.filter((item) => item.roles.includes(agent.roleCode));
    const NavContent = () => (<>
      ...
      <nav className="flex-1 px-3 py-4 space-y-1 overflow-y-auto">
        {filteredNavItems.map((item) => {
            ...
            return (<Link key={item.href} href={item.href} onClick={() => setIsOpen(false)} className={cn("flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-colors", isActive
      ...
      <div className="p-4 border-t border-gray-700 space-y-3">
        ...
        <form action={logout}>
          <Button type="submit" variant="ghost" className="w-full justify-center text-gray-300 hover:text-white hover:bg-gray-700">
