...
export function AdminSidebar({
  ...
  return (
    ...
    >
      ...
      {/* ── Header / Logo ────────────────────────────────── */}
      <div className={cn(
        "flex items-center border-b border-white/5 flex-shrink-0 h-[54px]",
        collapsed ? "justify-center px-2" : "justify-between px-4"
      )}>
        {/* Logo mark — always visible */}
        <div className="flex items-center gap-2.5 min-w-0">
          <div className="w-8 h-8 rounded-xl bg-gradient-to-br from-sky-500 to-cyan-400 flex items-center justify-center flex-shrink-0 shadow-lg shadow-cyan-500/30">
            <Waves className="w-[17px] h-[17px] text-white" />
          </div>
          ...
          >
            <p className="text-[13px] font-bold text-slate-100 tracking-tight whitespace-nowrap leading-none">
              {t("sidebar.title")}
            </p>
            <p className="text-[9.5px] font-bold text-cyan-400 uppercase tracking-[0.18em] mt-0.5 whitespace-nowrap">
              {t("sidebar.subtitle")}
            </p>
          </div>
          ...
          return (
            <div key={si} className={si > 0 ? "mt-1" : ""}>
              ...
              <div
                className={cn(
                  "px-4 py-1",
                  "text-[9.5px] font-bold uppercase tracking-[0.14em] text-[#253d56]",
                    ...
                    <li key={item.href} className="relative">
                      <Link
                        ...
                        className={cn(
                          ...
                          isActive
                            ? "bg-gradient-to-r from-cyan-500/12 to-sky-500/5 text-[#e2f0ff]"
                            : "text-[#4a6a88] hover:text-[#b8d5ee] hover:bg-white/[0.04]",
                          collapsed && "justify-center px-0 w-10 mx-auto"
                        )}
                      >
                        {/* Active indicator */}
                        {isActive && (
                          <span className="absolute left-0 top-[20%] h-[60%] w-[3px] rounded-r-full bg-gradient-to-b from-cyan-400 to-sky-500 shadow-[0_0_8px_rgba(6,182,212,0.7)]" />
                        )}

                        <Icon
                          className={cn(
                            "w-4 h-4 flex-shrink-0 transition-colors duration-150",
                            isActive ? "text-cyan-400" : "text-[#2e4a66] group-hover:text-[#7aabcc]"
                          )}
                          aria-hidden="true"
                        />
                        ...
                        {isActive && !collapsed && (
                          <span className="w-1.5 h-1.5 rounded-full bg-cyan-400 flex-shrink-0 shadow-[0_0_5px_rgba(6,182,212,0.8)]" />
                        )}
                        ...
                        {collapsed && (
                          <span
                            className={cn(
                              ...
                              "bg-[#0d1a2e]/95 backdrop-blur-sm border border-white/[0.08]",
                              "text-[#e2f0ff] px-2.5 py-1.5 rounded-lg text-[11.5px] whitespace-nowrap",
                          ...
                          >
                            {item.label}
                            <span className="absolute left-0 top-1/2 -translate-x-1.5 -translate-y-1/2 w-0 h-0 border-y-[5px] border-y-transparent border-r-[6px] border-r-[#0d1a2e]" />
                          </span>
                        )}
                      </Link>
                    </li>
      ...
      {/* ── Logout ────────────────────────────────────────── */}
      <div className="flex-shrink-0 border-t border-white/[0.05] p-2">
        <form action={handleLogout}>
          <button
            ...
            className={cn(
              ...
              "text-[#4a6a88] hover:text-red-400 hover:bg-red-500/8",
          ...
          >
            <LogOut
              className="w-4 h-4 flex-shrink-0 text-[#2e4a66] group-hover:text-red-400 transition-colors"
              aria-hidden="true"
            />
            <span
              className={cn(
