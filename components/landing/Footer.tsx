...
export async function Footer(props: {
    ...
    return (<footer id="contact" className="relative overflow-hidden bg-[#0a1628] text-white" style={{ fontFamily: "'DM Sans', sans-serif" }}>
      ...
      <div className="relative mx-auto max-w-7xl px-6 pb-8 pt-14 lg:px-8">
        <div className="grid gap-10 md:grid-cols-2 lg:grid-cols-4">
          ...
          <div className="lg:col-span-1">
            ...
            <div className="flex gap-2">
              {socialLinks.map((social, idx) => (<a key={idx} href={social.href} target="_blank" rel="noopener noreferrer" aria-label={social.ariaLabel} className="group flex h-8 w-8 items-center justify-center rounded-lg bg-white/5 text-slate-400 ring-1 ring-white/10 transition-all duration-200 hover:bg-cyan-500/15 hover:text-cyan-400 hover:ring-cyan-500/30">
                  <social.icon />
                </a>))}
            </div>
          </div>
          ...
          <div>
            ...
            <ul className="space-y-2.5">
              {quickLinks.map(({ href, label }) => (<li key={href}>
                  <IntlLink href={href} className="group flex items-center gap-2 text-sm text-slate-400 transition-colors hover:text-white">
        ...
        ].map((line, i) => (<li key={i} className="flex items-center gap-2 text-sm text-slate-400">
                  <span className={`h-1.5 w-1.5 rounded-full ${i === 2 ? "bg-slate-600" : "bg-cyan-500"}`}/>
                  {line}
                </li>))}
            </ul>
          </div>
