...
export function LandingNav() {
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      // Pick the section whose top is closest-but-still-above the sticky header.
      const headerOffset = 110;
      const nearBottom = headerOffset; // section must reach the header line
      const nearTop = headerOffset + 200; // allow sections whose top is a bit below header
      let bestSection = "";
      ...
      for (const id of sectionIds) {
        ...
        // Only consider sections that have started (not completely above)
        // and are close enough to the sticky header to highlight.
    ...
    <>
      ...
      >
        <div className="mx-auto flex h-16 max-w-7xl items-center justify-between px-4 sm:px-6 lg:px-8">
            ...
            {navLinks.map((link) => {
              ...
              return (
                ...
                >
                  ...
                  {isActive && (
                    <span className="absolute bottom-0 left-0 h-0.5 w-full bg-gradient-to-r from-cyan-500 to-cyan-600 dark:from-cyan-400 dark:to-cyan-500"></span>
                  )}
                </IntlLink>
              );
            })}
          ...
          {/* Desktop actions with enhanced buttons - FIXED SIZING */}
          <div className="hidden items-center gap-2 md:flex">
  ...
  cursor-pointer"
          ...
          <motion.button
            className="relative flex h-10 w-10 items-center justify-center rounded-md md:hidden hover:bg-gray-100 dark:hover:bg-gray-800"
        ...
        {/* Mobile Navigation with enhanced animation and styling */}
        <AnimatePresence>
          {isOpen && (
            <motion.div
                ...
                {navLinks.map((link, index) => {
                  ...
                  return (
                    ...
                    >
                      <IntlLink
                        href={link.href}
                        className="group flex items-center gap-3 rounded-lg px-3 py-2.5 text-base font-medium text-gray-600 transition-all duration-300 hover:bg-gradient-to-r hover:from-cyan-50 hover:to-transparent hover:pl-4 hover:text-cyan-600 dark:text-gray-300 dark:hover:bg-gradient-to-r dark:hover:from-cyan-950 dark:hover:to-transparent dark:hover:text-cyan-400 cursor-pointer"
                        onClick={() => {
      ...
      {scrolled && (
        <div className="fixed top-16 z-40 h-px w-full bg-gradient-to-r from-transparent via-cyan-500/20 to-transparent"></div>
      )}
    </>
  );
}