...
const SocialStats = ({ stats }: {
  ...
  if (!stats)
    return null;
  return (<div className="flex gap-4 mt-4 pt-4 border-t border-border">
...
export function TestimonialsSection({ className = "", showStats = true, autoplayInterval = 5000 }: TestimonialsSectionProps) {
  ...
  return (<section ref={ref} id="testimonials" className={`py-24 ${className}`} aria-label={t("ariaLabel")}>
    <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={isInView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.5, delay: 0.2 }}>
        <Carousel key={locale} opts={{
          ...
          <CarouselContent>
            {testimonials.map((testimonial, idx) => (<CarouselItem key={testimonial.id} className="md:basis-1/2 lg:basis-1/3">
              <motion.div onMouseEnter={() => setHoveredIndex(idx)} onMouseLeave={() => setHoveredIndex(null)} whileHover={{ y: -8, scale: 1.02 }} transition={{ type: "spring", stiffness: 300, damping: 20 }} className="p-2 h-full">
                <Card className="h-full border-border bg-card transition-all duration-500 overflow-hidden">
                  ...
                  <CardContent className="p-6 relative">
                    ...
                    {testimonial.tags && (<div className="flex flex-wrap gap-2 mb-3">
                      {testimonial.tags.map((tag, i) => (<Badge key={i} variant="secondary" className="text-xs bg-primary/5 text-muted-foreground">
                        {tag}
                      </Badge>))}
                    </div>)}
          ...
          <CarouselPrevious className="left-0 -translate-x-1/2 bg-card border-border shadow-md hover:bg-primary hover:text-primary-foreground transition-all duration-300" />
          <CarouselNext className="right-0 translate-x-1/2 bg-card border-border shadow-md hover:bg-primary hover:text-primary-foreground transition-all duration-300" />
        </Carousel>
      </motion.div>
